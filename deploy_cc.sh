...
CA_ADDRESS=ica-org1.example.com:7054  # 组织服务CA地址

ENV_MODE=install   # install or upgrade, 安装或者升级chain code
...
ENV_CHAIN_CODE_NAME=mycc
ENV_CHAIN_CODE_VERSION=1.0
ENV_CHAIN_CODE_PATH=github.com/hyperledger/fabric-samples/chaincode/marbles02/go
...
ENV_CORE_PEER_TLS_CLIENTKEY_FILE=/data/tls/peer0.org1.example.com-cli-client.key   # tls key
ENV_CORE_PEER_TLS_CLIENTCERT_FILE=/data/tls/peer0.org1.example.com-cli-client.crt  # tls 证书
ENV_PEER_ADDRESS_LIST=peer0.org1.example.com:7051,peer1.org1.example.com:7051              # 组织中的peer节点列表

# check cli container
checkCliContainer docker-compose-ca.yml

docker exec -e "ENV_MODE=${ENV_MODE}" -e "ENV_ADMIN_MSP_DIR=${ENV_ADMIN_MSP_DIR}" -e "ENV_CA_CHAINFILE=${ENV_CA_CHAINFILE}" \
...
-e "ENV_CORE_PEER_LOCALMSPID=${ENV_CORE_PEER_LOCALMSPID}" \
